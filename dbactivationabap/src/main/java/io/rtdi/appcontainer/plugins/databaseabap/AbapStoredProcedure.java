...
public class AbapStoredProcedure implements IStoredProcedure {
    ...
    public ObjectNode callProcedure(
		...
		if (metadata == null) {
			metadata = new AbapProcedureMetadata();
			// Output parameters
			...
			try (PreparedStatement stmt = conn.prepareStatement(selectsql); ) {
				...
				try (ResultSet rs = stmt.executeQuery(); ) {
					...
					while (rs.next()) {
						...
						sql.append(AbapDataTypes.getDataTypeString(rs.getString(2), rs.getInt(3), rs.getInt(4)));
