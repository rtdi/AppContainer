    ...
    public static ObjectNode callProcedure(
		...
		while (iter.hasNext()) {
			...
			if (data.get(fieldname).getNodeType() == JsonNodeType.ARRAY) {
				...
				if (p == null) {
					throw new AppContainerSQLException("The procedure does not have a parameter \"" + fieldname + "\"", null);
				} else if (!(p instanceof TableParameterMetadata)) {
					throw new AppContainerSQLException("The input for the parameter \"" + fieldname + "\" is not a table, hence no Json array is allowed as input", null);
				} else {
