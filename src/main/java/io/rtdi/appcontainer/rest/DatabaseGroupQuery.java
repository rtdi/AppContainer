...
@Path("/")
public class DatabaseGroupQuery {
					...
					try (ResultSet rs = stmt.executeQuery(); ) {
						...
						while (rs.next()) {
							...
							while (i < levelcolumns) {
								Object v = DatabaseQuery.getJsonValue(rs, i+1, timeformatter, timestampformatter);
								if (v != null) {
							...
							while (i < columncount) {
								DatabaseQuery.setRestObject(rs, i+1, leaf.jsonnode, timeformatter, timestampformatter);
								i++;
							}
							rowcount++;
							if (rowcount > 30000) {
